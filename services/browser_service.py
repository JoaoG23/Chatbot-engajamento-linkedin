...
logger = setup_logger("BrowserService")
...
class BrowserService:
    ...
    async def _safe_goto(page: Page, url: str, maximum_retries: int = 3) -> None:
        ...
        for attempt_index in range(maximum_retries):
            try:
                ...
                logger.warning(
                    f"Tentativa {attempt_index + 1}/{maximum_retries} de navegação para {url} falhou: {navigation_error}"
                )
    ...
    async def connect_existing_chrome(
        ...
        logger.info(f"Conectando ao Chrome em execução via CDP ({cdp_url})...")
        ...
        logger.info(f"Conectado à página: {await active_page.title()}")
    ...
    async def launch_new_browser(
        ...
        logger.info("Iniciando nova instância do navegador no modo anônimo (incognito)...")
    ...
    async def do_login(page: Page, email: str, password: str) -> None:
        ...
        logger.info(f"Navegando para {LINKEDIN_HOME_URL}...")
        ...
        if "feed" in page.url:
            logger.info("Sessão iniciada no Feed do LinkedIn com sucesso!")
            return

        logger.info("Login efetuado. Se houver verificação ou 2FA, conclua na janela aberta.")
